...
export class ConceptualMapComponent {
   ...
   private readonly C = {
      ...
      L2_JCX: 119,
      L3_X: 143,
      L1_W: 290, // ancho fijo de branches L1
      L2_W: 280, // ancho fijo de modules L2
      L3_W: 280, // ancho fijo de units L3
      CHIP_W: 280,
   };
   ...
   private buildLayout(): MapLayout {
      ...
      const vpath = (x: number, y1: number, y2: number): string => `M${x} ${y1} L${x} ${y2}`;
      ...
      for (let i = branchLayouts.length - 1; i >= 0; i--) {
         ...
         paths.push({
            d: lpath(C.SPINE_X, chipBottom, C.L1_X, bl.centerY),
            color: bl.color,
            dashed: false,
         });
      }
      ...
      for (const bl of branchLayouts) {
         ...
         nodes.push({
            ...
            width: C.L1_W,
            height: C.NODE_H,
            hasChildren: branch.modules.length > 0,
         });
         ...
         if (exp[branch.id] && mods.length > 0) {
            ...
            for (let mi = mods.length - 1; mi >= 0; mi--) {
               paths.push({
                  d: lpath(C.L1_JCX, jointCy, C.L2_X, mods[mi].centerY),
                  color,
                  dashed: false,
               });
            }
            ...
            for (const { mod, y: mY, centerY: mCY, bottom: mBtm, jointCy: mJCy, units } of mods) {
               ...
               if (exp[mod.id] && units.length > 0) {
                  ...
                  for (let ui = units.length - 1; ui >= 0; ui--) {
                     paths.push({
                        d: lpath(C.L2_JCX, mJCy, C.L3_X, units[ui].centerY),
                        color,
                        dashed: true,
                     });
                  }
   ...
   getNodeStyle(node: RenderedNode): string {
      ...
      switch (node.level) {
         case 0:
            return `${base}justify-content:center;padding:0 14px;border-radius:14px;border:5px solid #00bcff;background:white;`;
         case 1:
            return `${base}padding:0 14px;border-radius:11px;background:${c};cursor:pointer;`;
         case 2:
            return `${base}padding:0 12px;border-radius:11px;background:white;border:4px solid ${c};cursor:pointer;`;
         case 3:
            return `${base}padding:0 12px;border-radius:11px;background:white;border:4px dotted ${c};`;
         default:
            return base;
      }
   }
   ...
   getTextStyle(node: RenderedNode): string {
      switch (node.level) {
         case 0:
            return 'font-size:13px;font-family:Poppins,sans-serif;font-weight:bold;color:#3d474d;white-space:normal;overflow:hidden;text-overflow:ellipsis;';
         case 1:
            return 'font-size:16px;font-family:Poppins,sans-serif;color:white;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;';
         case 2:
            return 'font-size:16px;font-family:Poppins,sans-serif;color:#3d474d;white-space:normal;overflow:hidden;text-overflow:ellipsis;';
         case 3:
            return 'font-size:14px;font-family:Poppins,sans-serif;font-weight:bold;color:#6b7280;white-space:normal;overflow:hidden;text-overflow:ellipsis;';
         default:
            return '';
      }
   }
   ...
   onWheel(e: WheelEvent): void {
      ...
      this.zoom.update((z) =>
         Math.min(this.ZOOM_MAX, Math.max(this.ZOOM_MIN, +(z + d).toFixed(2))),
      );
   }
